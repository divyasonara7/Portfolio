<template>
  <!-- banner section -->
  <section id="home" class="w3l-banner py-5">
    <div class="container">
      <div class="row align-items-center">
        <div class="col-lg-6 col-sm-12 mt-lg-0 mt-4">
          <span class="title-small">Hello</span>
          <h1 class="mb-2 title"><span>I'm</span> Divya Sonara</h1>
          <h1 class="mb-4 title">
            a <span class="typed-text">Web developer.</span>
            <!-- <span class="cursor">&nbsp</span> -->
          </h1>
          <p>I design and code beautifully simple things, and I love what I do.</p>
          <div class="mt-sm-5 mt-4">
            <a class="btn btn-primary btn-style mr-2" href="contact.html"> Hire Me </a>
            <a class="btn btn-outline-primary btn-style mr-2" href="#"> Portfolio </a>
          </div>
        </div>
        <div class="col-lg-6 col-md-8 col-sm-10 mt-lg-0 mt-4">
          <div class="img-effect text-lg-center">
            <img src="../assets/images/logo.png" alt="myphoto" class="img-fluid" />
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- //banner section -->
</template>
